...
pub fn authenticate_user(ctx: &Context, id: Option<Uuid>) -> Result<()> {
...
/// Only regards user roles if `id` parameter is `None`.
pub fn check_permissions(
    authenticate_user_header: &AuthorizedUserHeader,
    id: Option<Uuid>,
) -> Result<()> {
    let id_contained_in_header = id
        .and_then(|id| Some(authenticate_user_header.id == id))
        .unwrap_or(false);
